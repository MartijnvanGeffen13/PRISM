...
#   $env:EXCHANGE_WEBHOOK_URL  = "https://<func>.azurewebsites.net/api/webhook?code=<function-key>"
$tenantId     = $env:PURVIEW_TENANT_ID
$clientId     = $env:PURVIEW_CLIENT_ID
$clientSecret = $env:PURVIEW_CLIENT_SECRET
$webhookUrl   = $env:EXCHANGE_WEBHOOK_URL

foreach ($v in @("PURVIEW_TENANT_ID", "PURVIEW_CLIENT_ID", "PURVIEW_CLIENT_SECRET", "EXCHANGE_WEBHOOK_URL")) {
...
# Webhook endpoint (must be public HTTPS) comes from $env:EXCHANGE_WEBHOOK_URL (set above).
